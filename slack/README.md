...
このフォルダには、ブリッジアプリと Slack を連携させるための設定ファイルが含まれています。
...
## 2. URL の書き換え
マニフェスト内の以下の箇所を、自分の Code Engine の URL(`https://~.appdomain.cloud`) に書き換えてください。URLの末尾に/slack/eventsを付与します。
- `settings` > `event_subscriptions` > `request_url`
  - 形式: `[YOUR_CODE_ENGINE_URL]/slack/events`

[Next]>[Create]tと操作しアプリケーション作成を完了します。
## 3. 認証情報の取得
アプリのセットアップ画面の **Install APP** タブで表示される **[Install to _Workspace_]** を開き **[許可する]** を行ったたら、以下の 2 つの値をメモして Code Engine の環境変数に設定してください。
...
## 4. チャンネル ID の取得
有人エージェント接続チャットを行いたい Slack チャンネルの ID を取得します。
1. 対象のチャンネルを右クリック ＞ [チャネル詳細を表示]
2. 表示されたチャネル詳細ページの最下部の **チャネルID**ID**`C0123456789`）をメモし、Code Engine の `SLACK_CHANNEL_ID` に設定してください。

## 5. チャンネルへのアプリ招待
設定が完了しても、アプリがチャンネルに参加していないとメッセージの送受信ができません。

1.  **アプリを招待する**
    - 通知・チャットを行う Slack チャンネル（`SLACK_CHANNEL_ID` で指定したもの）を開きます。
    - メッセージ入力欄で `/invite @wxa-wxo-agent `を入力・実行し、アプリをチャンネルに参加させす。

## 6. Code Engine への環境変数設定
